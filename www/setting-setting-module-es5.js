(function () {
  ...
  (window["webpackJsonp"] = window["webpackJsonp"] || []).push([["setting-setting-module"], {
    ...
    function srcAppSettingSettingPageTs(module, __webpack_exports__, __webpack_require__) {
      ...
      var _alerterror_service__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! ../alerterror.service */
      ...
      var SettingPage = /*#__PURE__*/function () {
        function SettingPage(menuCtrl, toastController, restService, storage, alertService, firebase) {
        ...
        _createClass(SettingPage, [{
          ...
          value: function ngOnInit() {
            ...
            this.storage.get('perTimeslotBookings').then(function (perTimeslotBookings) {
              _this.storage.get('allowDiscount').then(function (allowDiscount) {
                _this.perTimeslotBookings = perTimeslotBookings;
                _this.allowDiscount = allowDiscount;
              });
            });
            this.storage.get('restaurants_id').then(function (restaurants_id) {
              ...
              _this.restService.setting(stringy).subscribe(function (response) {
                _this.getSetting = JSON.parse(response['_body']);
                console.log('setting:  ' + _this.getSetting);
                _this.perTimeslotBookings = _this.getSetting.settings.per_time_slot_booking_cont;
                _this.address = _this.getSetting.settings.location;
                console.log(_this.address.settings);
                _this.openingTime = _this.getSetting.settings.opening_time;

                if (_this.getSetting.settings.allow_discounts == 'Yes') {
                  _this.allowDiscount = true;
                } else {
                  _this.allowDiscount = false;
                }
              }, function (err) {
          ...
          value: function updatebtn() {
            ...
            this.storage.get('restaurants_id').then(function (restaurants_id) {
              if (_this2.allowDiscount == true) {
                _this2.allowDiscounts = 'Yes';
              } else {
                _this2.allowDiscounts = 'No';
              }

              console.log(_this2.allowDiscounts);
              var stringy = JSON.stringify({
                requestType: 'update_settings',
                restaurantId: restaurants_id,
                perTimeslotBookings: _this2.perTimeslotBookings,
                allowDiscounts: _this2.allowDiscounts,
                openingTime: _this2.openingTime,
                address: _this2.address
              });

              _this2.restService.updateRestaurantSettings(stringy).subscribe(function (response) {
                _this2.settingData = JSON.parse(response['_body']);
                console.log(_this2.settingData);

                if (_this2.settingData.status == 'success') {
                  _this2.storage.set('perTimeslotBookings', _this2.perTimeslotBookings);

                  _this2.storage.set('allowDiscount', _this2.allowDiscount);
